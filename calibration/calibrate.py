...
def sigmoid_fit(calibration, fit_name, params, graph=True):
    ...
    if graph:
        graph_2d_data(
            ...
            "sigmoid",
            0,
            max([max(sublist) for sublist in measured_data]),
            500,
        )
...
def three_dimension_fit(calibration, fit_name, params, graph=True):
    ...
    for i in range(16):
        ...
        fitted_parameters, pcov = scipy.optimize.curve_fit(
            three_dim, [x_data, y_data], z_data, p0=initial_parameters
        )
...
def graph_2d_data(
    ...
    for i in range(16):
        ax[i // 4, (i % 4)].plot(
            measured_data[i], medians[i], "o", markersize=3, color="black"
        )
        ax[i // 4, (i % 4)].errorbar(
            measured_data[i], medians[i], yerr=standard_deviations[i], fmt="none"
        )
        ax[i // 4, (i % 4)].plot(
            linear_space,
            func(linear_space, *coefficients[i]),
            markersize=1.5,
            label=None,
        )
...
def graph_3d_data(func, datas, coefficients, fit_name):
    ...
    for i, data in enumerate(datas):
        ...
        ax.plot_surface(
            X, Y, Z, rstride=1, cstride=1, linewidth=1, antialiased=True, alpha=0.5
        )

        ax.scatter(
            x_data, y_data, z_data, c="r", s=10
        )  # show data along with plotted surface
...
def process_vial_data(calibration, param=None):
    ...
    for i, vial_data in enumerate(vial_datas):
        ...
        calibration_data[names[i]] = {
            ...
            "measured_data": calibration["measuredData"],
        }
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-a", "--ip", action="store", dest="ipaddress", help="IP address of eVOLVER"
    )
    parser.add_option(
        "-t",
        "--fit-type",
        action="store",
        dest="fittype",
        help="Valid options: sigmoid, linear, constant, 3d",
    )
    parser.add_option(
        "-f",
        "--fit-name",
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="alwaysyes",
        help="Skips asking to save calibration to eVOLVER",
    )
    parser.add_option(
        "-r",
        "--no-graph",
        action="store_true",
        dest="nograph",
        help="Skips graphing if provided",
    )
    ...
    if cal_name:
        ...
        if no_graph is None:
            no_graph = False
        dpu_evolver_ns.emit(
            "getcalibration", {"name": cal_name}, namespace="/dpu-evolver"
        )
