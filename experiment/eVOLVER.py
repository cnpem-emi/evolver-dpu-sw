...
if __name__ == '__main__':
    ...
    while(True):
        try:
            while(True):
                ...
                # command = {"command": bytes, "payload": bytes, "reply": boolean}
            ...
            try:
                ...
                while True:
                    key = input('Experiment paused. Press enter key to restart or hit Ctrl-C again to terminate experiment')
    ...
    EVOLVER_NS.connect()
    EVOLVER_NS.stop_ex#!/usr/bin/env python3
...
lock = (
    Lock()
)  # Lock is similar to mutex. Will only 'allow' one task to use same socket at a time
...
class EvolverDPU:
    ...
    def broadcast(self, data: dict):
        ...
        if data is None:
            logger.error(
                "could not tranform raw data, skipping user-" "defined functions"
            )
            return
        ...
        try:
            ...
            for param in od_cal["params"]:
                # Order raw data before saving values
                for ss in range(16):
                    raw_data[ss] = data["data"].get(param, [])[
                        channelIdx[str(ss)]["channel"]
                    ]
                self.save_data(raw_data, elapsed_time, VIALS, param + "_raw")
    ...
    def transform_data(self, data, vials, od_cal, temp_cal):
        ...
        for x in vials:
            ...
            try:
                ...
                temp_value[x] = (
                    float(temp_value[x]) * temp_coefficients[0]
                ) + temp_coefficients[1]
                # print('temperature from vial %d: %.3f' % (x, temp_value[x]))
            except ValueError:
                print("Temp Read Error")
    ...
    def initialize_exp(
        self, vials, experiment_params, log_name, quiet, verbose, always_yes=False
    ):
        self.experiment_params = experiment_params
        logger.info("initializing experiment")
        ...
        if os.path.exists(EXP_DIR):
            ...
            if always_yes:
                ...
                while exp_continue not in ["y", "n"]:
                    exp_continue = (
                        "n"  # input('Continue from existing experiment? (y/n): ')
                    )
        else:
            exp_continue = "n"
        ...
        if exp_continue == "n":
            if os.path.exists(EXP_DIR):
                ...
                if exp_overwrite == "y":
                    ...
                    print(
                        "Change experiment name in custom_script.py "
                        "and then restart..."
                    )
                    logger.warning("not deleting existing data directory, exiting")
                    sys.exit(1)
            ...
            if self.experiment_params:
                stir_rate = list(
                    map(
                        lambda x: x["stir"],
                        self.experiment_params["vial_configuration"],
                    )
                )
                temp_values = list(
                    map(
            ...
            with open(TEMP_CAL_PATH) as f:
                ...
                raw_temperatures = [
                    str(
                        int(
                            (temp_values[x] - temp_coefficients[x][1])
                            / temp_coefficients[x][0]
                        )
                    )
                    for x in vials
                ]
        ...
        # copy current custom script to txt file
        backup_filename = "{0}_{1}.txt".format(EXP_NAME, time.strftime("%y%m%d_%H%M"))
        shutil.copy(
    ...
    while True:
        try:
            while True:
                ...
                command = json.loads(
                    command[1].decode("UTF-8", errors="ignore").lower()
                )
            ...
            try:
                ...
                while True:
                    key = input(
                        "Experiment paused. Press enter key to restart "
